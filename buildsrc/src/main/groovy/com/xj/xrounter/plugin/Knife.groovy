...
class Knife {
    static void handle() {
        println("Knife handle")
        File targetFile = XRouterTransform.registerTargetFile
        ...
        if (targetFile.name.endsWith(".jar")) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                jarFile.getInputStream(jarEntry).withCloseable { is ->
                    if (entryName == Scanner.REGISTER_CLASS_NAME) { // find XRouter.class
            ...
            if (opcode == Opcodes.RETURN) { // handle init code before return
                Scanner.records.each { record ->
                    record.aptClasses.each { className ->
                        ...
                        mv.visitFieldInsn(Opcodes.GETSTATIC, "com/xj/xrouter/api/XRouter", getFieldNameByInterface(record), "Ljava/util/Map;")
                        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, className, "handleMap", "(Ljava/util/Map;)V", false)
                    }
                }
            }
